...
def write_transcript(path: Path, rows: list[tuple[int, int, str]]) -> Path:
    ...
    chunk_size = 200
    chunks = []
    for chunk_id, start_index in enumerate(range(0, len(segments), chunk_size)):
        chunk = segments[start_index:start_index + chunk_size]
        chunks.append({
            "chunk": chunk_id,
            "segment_start": chunk[0]["segment"],
            "segment_end": chunk[-1]["segment"],
...
def load_transcript_jsonl(path: Path) -> list[dict]:
    ...
    for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        required = {"segment", "start_ms", "end_ms", "text"}
...
def transcript_slice(args: argparse.Namespace) -> None:
    ...
    if all(value is None for value in (
        ...
        raise SystemExit("Provide a time or segment boundary for transcript-slice.")
    segments = load_transcript_jsonl(source)
    selected = []
    for segment in segments:
        index = int(segment["segment"])
        if args.segment_start is not None and index < args.segment_start:
            continue
        if args.segment_end is not None and index > args.segment_end:
            continue
    ...
    lines = [
        ...
        "",
        f"- Source: `{source}`",
        f"- Segments: `{selected[0]['segment']}–{selected[-1]['segment']}` of `{len(segments)}`",
